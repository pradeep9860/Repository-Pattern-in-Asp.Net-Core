using Application.Core.Entity; 
using Core.DI.Base;
using Core.Entity;
using Core.EntityDto;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace Application.Service.Providers
{
    public class CompanyService : ITransientService, ICompanyService
    {
        private readonly IUnitOfWork _uow;
        private readonly IRepository<Company> _repository;
        private readonly IRepository<Division> _divisionRepository;

        public CompanyService(IUnitOfWork unit, IRepository<Company> repository, IRepository<Division> divisionRepository)
        {
            _uow = unit;
            _repository = repository;
            _divisionRepository = divisionRepository;
        }

        public List<CompanyDto> GetAll(string searchText)
        {
            var result = _repository.AsQueryable();
            if (!string.IsNullOrEmpty(searchText))
            {
                result = result.Where(x => x.NAME.ToLower().Trim().Contains(searchText));
            }

            return result.Select(x => new CompanyDto {
                NAME = x.NAME,
                ACID = x.ACID,
                ADDRESS = x.ADDRESS,
                EMAIL = x.EMAIL,
                FAX = x.FAX,
                FBDATE = x.FBDATE,
                FBDATE_BS = x.FBDATE_BS,
                FEDATE = x.FEDATE,
                FEDATE_BS = x.FEDATE_BS,
                ID = x.ID,
                INITIAL = x.INITIAL,
                WEBSITE = x.WEBSITE,
                ISBRANCH = x.ISBRANCH,
                PhiscalID = x.PhiscalID,
                TELA = x.TELA,
                VAT = x.VAT
            }).ToList();
        }

        public void Update(CompanyDto entity)
        {
            //use automapper 
            var saveModel = new Company
            { 
                NAME = entity.NAME,
                ACID = entity.ACID,
                ADDRESS = entity.ADDRESS,
                EMAIL = entity.EMAIL,
                FAX = entity.FAX,
                FBDATE = entity.FBDATE,
                FBDATE_BS = entity.FBDATE_BS,
                FEDATE = entity.FEDATE,
                FEDATE_BS = entity.FEDATE_BS,
                ID = entity.ID,
                INITIAL = entity.INITIAL,
                WEBSITE = entity.WEBSITE,
                ISBRANCH = entity.ISBRANCH,
                PhiscalID = entity.PhiscalID,
                TELA = entity.TELA,
                VAT = entity.VAT
            };

            _repository.Update(saveModel);
            _uow.Commit(); 
        }

        public void Save(CompanyDto entity)
        {
            var saveCompanyModel = new Company
            {
                NAME = entity.NAME,
                ACID = entity.ACID,
                ADDRESS = entity.ADDRESS,
                EMAIL = entity.EMAIL,
                FAX = entity.FAX,
                FBDATE = entity.FBDATE,
                FBDATE_BS = entity.FBDATE_BS,
                FEDATE = entity.FEDATE,
                FEDATE_BS = entity.FEDATE_BS,
                ID = entity.ID,
                INITIAL = entity.INITIAL,
                WEBSITE = entity.WEBSITE,
                ISBRANCH = entity.ISBRANCH,
                PhiscalID = entity.PhiscalID,
                TELA = entity.TELA,
                VAT = entity.VAT
            };

            _repository.Add(saveCompanyModel);

            var saveDivisionModel = new Division
            {
                INITIAL = entity.INITIAL,
                COMID = entity.ID,
                COMNAME = entity.NAME
            };
            _divisionRepository.Add(saveDivisionModel);

            _uow.Commit();

        }

        public bool CheckCompany()
        {
            return _repository.AsQueryable().Any();
        }
    }
}
